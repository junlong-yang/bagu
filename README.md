...
4. 滑动窗口协议。累积确认，发送方的数据保留在缓冲区一段时间，直到收到该报文段的确认应答，或后续报文段的确认应答。发送方和接收方都存在滑动窗口。
5. 流量控制。根据接收方的缓冲区大小，发送方进行流量控制。
6. 拥塞控制。在出现网络拥塞之后，通过控制拥塞窗口的大小，调节发送方发送的数据量，以降低网络的拥塞程度。

## 浏览器发送出一个请求到收到响应经历了哪些步骤
1. 浏览器解析用户输入的URL，生成一个HTTP格式的请求。
2. 根据URL域名从本地hosts文件查找映射IP，如果没有就会发送给电脑配置的DNS进行域名解析，得到IP地址。
...
- 死循环：JDK1.7中，hashMap在并发put中会出现死循环的问题。
...
- 假如只重写equals而不重写hashcode，那么类的hashcode方法就是Object默认的hashcode方法，由于默认的hashcode方法是根据对象的内存地址经哈希算法得来的，那么两个对象的hashcode不一定相等。
- 然而重写了equals，导致进行equals比较的时候两个对象相等，根据hashcode的规则，两个对象相等其哈希值一定相等，所以矛盾就产生了，因此重写equals一定要重写hashcode。
- 在map里面，先判断hashCode是否相同，再去调用equals判断对象相同，因此重写了equals方法，一定要以相关联的规则重写hashcode。
...
7. G1收集器：是一款以Region(区域)为最小内存回收单位的，逻辑上采用分代收集理论的垃圾收集器。G1收集器有点众多，但也有一些缺点，如果应用的内存非常吃紧，对内存进行部分回收根本不够，始终要进行整个Heap的回收，那么G1要做的工作量就一点也不会比其它垃圾回收器少，而且因为本身算法复杂了一点，可能比其它回收器还要差。因此G1比较适合内存稍大一点的应用(一般来说至少4G以上)，小内存的应用还是用传统的垃圾回收器比如CMS比较合适。
...
- 32位处理器可控内存空间有4GB,但是具体的操作系统会给一个限制，这个限制一般是2GB-3GB（一般来说Windows系统下为1.5G-2G，Linux系统下为2G-3G），而64bit以上的处理器就不会有限制了。
...
## 如何进行JVM调优
1. JVM调优目标：任何一个属性性能的提高，几乎都是以牺牲其他属性性能的损为代价的，不可兼得，具体根据在业务中的重要性确定。
   - 内存占用
   - GC低停顿和GC低频率低
   - 高吞吐量
2. 量化目标：以下举例。
...
3. 调优方式
   - 内存配置
      - 默认空余堆内存小于40%时，JVM就会增大堆直到-Xmx的最大限制；空余堆内存大于70%时，JVM会减少堆直到-Xms的最小限制。因此服务器一般设置-Xms、-Xmx相等以避免在每次GC后调整堆的大小。
      - 观察FullGC后，老年代通常的内存大小，在服务器条件允许的情况下，可以配置最大堆内存为老年代空间占用的3-4倍。
      - 例如32G的服务器，默认最大堆内存为物理内存的1/4(8G)，按照比例，老年代5G，年轻代3G。假设FullGC后老年代还占用4G，那么可以将最大堆内存配置为12-16G，其中老年代8-12G，基本保证老年代内存使用率低于70%。
   - GC配置
      - 选择合适的GC策略，CMS（低时延）、ParrelGC（高吞吐）、G1（大内存）。
      - 配置接受的STW停顿时间。
   - 代码分析
      - 通过jmap -histo 观察占用内存较多的对象，分析是否可以优化。
      - 使用 jstat -gcxxx pid 查看垃圾回收情况，特别是fullgc比较频繁，就需要调优了，可能是对象太大不能放到年轻代。
...
2. @Bean注解：得到bean对象。
3. @Controller、@Service、@Repository、@Autowired、@Resource、@Qualifier、@Transactional等。
...
2. bin log：  
    bin log是Server层的日志，记录了原始的sql语句。  
3. undo log：  
...
## ACID是什么，如何保证
- A，原子性，由undo log保证，它记录了需要回滚的日志信息，事务回滚时撤销已经执行的动作。
- C，一致性，由其他三大特征保证，程序代码要保证业务上的一致性。
- I，隔离性，又MVCC保证。
- D，持久性，由内存+redo log保证保证数据的写入和恢复能力。
...
## 最左前缀原则是什么
对于联合索引a,b,c，要使用a字段进行筛选，才能使用到联合索引。因为非叶子节点存储的是第一个关键字的索引，而叶节点是按照a,b,c这个顺序比较大小进行排序的。
